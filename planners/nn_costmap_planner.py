...
class NNCostmapPlanner(NNPlanner):
    """ A differentiable planner which selects an optimal waypoint 
    (ie. spline trajectory) using a neural network trained to output
    a costmap, or a list of costs per candidate waypoint. """

    def __init__(self, simulator, params):
        super(NNCostmapPlanner, self).__init__(simulator, params)
    ...
    def _raw_data(self, start_config):
        ...
        costmap_length = self.params.data_creation.num_candidate_waypoints + 1
        data['costmap'] = np.ones((costmap_length, 4), dtype=np.float32)
        return data
